...
import { reinforceUIConfig } from '../ui/metricsConfigs/reinforce';
import type { ReinforceMetrics } from '../ui/metricsConfigs/reinforce';
...
const metrics: ReinforceMetrics = {
    ...
    currentCriticLoss: 0, currentAdvantage: 0, currentEntropy: 0, currentGradientClipRate: 0,
    actionHistory: [], currentMean: 0, currentStd: 0,
};

const worker = new Worker(new URL('../training/workers/reinforceSingle.worker.ts', import.meta.url), { type: 'module' });

let latestFrame: RenderableEnvironment | null = null;
let renderEnabled = true;

const panel = new MetricsPanel(sidebar, metrics, reinforceUIConfig, {
...
worker.onmessage = (e: MessageEvent) => {
    ...
    switch (msg.type) {
        ...
        case 'brainData': {
            ...
            a.href = url;
            a.download = `reinforce-single-pendulum-ep${episode}.json`;
