...
import { dqnUIConfig } from '../ui/metricsConfigs/dqn';
import type { DQNMetrics } from '../ui/metricsConfigs/dqn';
...
const metrics: DQNMetrics = {
    ...
    currentLoss: 0, currentQ: 0, lossHistory: [], qValueHistory: [],
    latestQValues: [], currentActionIndex: 0, epsilon: 1,
};

const worker = new Worker(new URL('../training/workers/dqnDouble.worker.ts', import.meta.url), { type: 'module' });

let latestFrame: RenderableEnvironment | null = null;
let renderEnabled = true;

const panel = new MetricsPanel(sidebar, metrics, dqnUIConfig, {
...
worker.onmessage = (e: MessageEvent) => {
    ...
    switch (msg.type) {
        ...
        case 'brainData': {
            ...
            a.href = url;
            a.download = `dqn-double-pendulum-ep${episode}.json`;
